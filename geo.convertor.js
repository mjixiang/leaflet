(function () {
    ...
    function wgs84ToBd09_BaiduAPI(location) {
        var callbackName = 'cbk_' + Math.round(Math.random() * 10000);    //随机函数名
        var xyUrl = "http://api.map.baidu.com/ag/coord/convert?from=0&to=5&x=" + location.lng + "&y=" + location.lat + "&callback=GIS.Baidu." + callbackName;
        //动态创建script标签
        load_script(xyUrl);
        var result = null;
        GIS.Baidu[callbackName] = function (xyResult) {
            delete BMap.Convertor[callbackName];    //调用完需要删除改函数
            result = { lat: xyResult.y, lng: xyResult.x };
        }
